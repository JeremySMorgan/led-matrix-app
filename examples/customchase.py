...
"""
try:
    numPixels = 28 * 28  # 1/2 Strip plus one LED is broken
    strip = apa102.APA102(
        numPixels, brite
    )  # Low brightness (2 out of max. 31)
        ...
        for j in range(256):  # Change the color through the color wheel
            for q in range(7):
                ...
                for i in range(
                    ...
                    strip.setPixelRGB(i + q + 6, index)
                    # Wrap, if we are at the end of the strip
                strip.show()
                sleep(pause)
