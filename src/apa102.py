...
class APA102:
    ...
    def __init__(
        self, numLEDs, globalBrightness=31
    ...
    def setPixel(self, ledNum, red, green, blue):
        """Sets the color of one pixel in the LED stripe. The changed
        pixel is not shown yet on the Stripe, it is only written to the
        pixel buffer. Colors are passed individually.
        """
        if ledNum < 0:
